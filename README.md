...
一个基于 Avalonia 的跨平台媒体重命名工具，支持在 Windows 和 macOS 上通过拖拽图片或 MOV 文件进行原地重命名。
...
- 如果目标文件名已存在，会自动添加 `_001`、`_002` 等后缀，例如 `20240521_153012(img_0001)_001.jpg`。
- 单个文件处理失败不会中断其他文件。

## 支持格式

工具会优先支持常见图片格式，并支持 iOS 实况照片伴随的 MOV 文件，包括：
...
部分格式可能没有可读取的元数据时间，此时会自动使用文件创建时间命名。
...
注意：这是原地重命名工具，不会生成备份文件。处理重要媒体文件前，建议先确认已有备份。
...
- MetadataExtractor：读取图片 EXIF 和 MOV QuickTime 元数据
- xUnit：单元测试
